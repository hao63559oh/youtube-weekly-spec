...
- **精度チューニングの入口**：
  ...
  - 条件（`config.json`）→ `discovery.keywords`（現在 CM/MV/その他 各4語）、`genre_ratio`（cm:mv:other=0.4/0.4/0.2）、`max_per_channel`(2)、`min/max_duration_sec`(15/600)、`score_threshold`(65)、`max_candidates`(80)、`recent_exclude`（`video_days`=3650・`channel_days`=30・`enabled`）。
